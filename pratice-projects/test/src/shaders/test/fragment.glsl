...
void main() {
    ...
    // Pattern 11
    // float strength = max(mod(vUv.y * 10.0, 1.0), mod(vUv.x * 10.0, 1.0));
    // strength = step(0.8, strength);
    ...
    // Pattern 16
    // float strength = abs(cos(vUv.x * PI)) / 2.0;
    ...
    // Pattern 25
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor(vUv.y * 10.0 + vUv.x * 4.0) / 10.0);
    // float strength = random(gridUv);
    ...
    // float strength = 1.0 - step(0.01, abs(length(waveUv - 0.5) - 0.25));
    ...
    // Pattern 42
    // vec2 converted = vUv - 0.5;
    // float angle = atan(converted.x, converted.y);
    // angle /= PI;
    // angle = (angle + 1.0) / 2.0;
    // float strength = angle;
    ...
    // Pattern 48
    // float strength = 1.0 - abs(step(0.0, cnoise(vUv * 10.0)));
